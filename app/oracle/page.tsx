...
function buildMarkdownFromTimeline(entries: TimelineEntry[]): string {
  if (entries.length === 0) {
    return "# Oracle – Last 7 Days\n\n_No entries in the last seven days._\n";
  }

  let md = "# Oracle – Last 7 Days\n\n";

  for (const { dateKey, entry } of entries) {
    md += `## ${formatDateLabel(dateKey)}\n\n`;

    if (entry.signal.trim()) {
      md += `**Signal**\n\n${entry.signal.trim()}\n\n`;
    }
    if (entry.friction.trim()) {
      md += `**Friction**\n\n${entry.friction.trim()}\n\n`;
    }
    if (entry.alignment.trim()) {
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500">
              Oracle · Mirror {host === "oracle.solaces.me" && "· Subdomain"}
            </p>
      ...
      <section className="rounded-xl border border-slate-800 bg-slate-900/60 px-4 py-4 sm:px-5 sm:py-5 space-y-3">
        <div className="flex items-center justify-between gap-2">
          ...
          {recentEntries.length > 0 && (
            <button
              onClick={() => {
                const md = buildMarkdownFromTimeline(recentEntries);
                const todayKey = getTodayKey();
                downloadTextFile(`oracle-last-7-${todayKey}.md`, md);
              }}
              className="text-[0.7rem] rounded-md border border-slate-700 px-3 py-1 hover:bg-slate-800 text-slate-200"
            >
              Export as .md
            </button>
          )}
